...
class _MatchingPicturesScreenState extends State<MatchingPicturesScreen> {
    ...
    try {
      ...
      for (final key in manifestMap.keys) {
        ...
        if (lowerKey.startsWith('assets/images/fruits/') &&
          ...
          try {
            ...
            // If this asset lives in a 'cartoon' subfolder, register it separately
            if (preferred.toLowerCase().contains('/fruits/cartoon/')) {
              fruitsCartoon.add(preferred);
            } else {
      ...
      try {
        final keys = manifestMap.keys
            .where((k) => k.toLowerCase().contains('/fruits/'))
            .toList();
        if (keys.isNotEmpty) {
          final sampleKeys = keys.take(24).join(', ');
          debugPrint(
              'PicturesScreen: assetManifest fruits sample count=${keys.length}');
          debugPrint(
              'PicturesScreen: assetManifest fruits sample: $sampleKeys');
        }
        final cartoonKeys = manifestMap.keys
            .where((k) => k.toLowerCase().contains('/fruits/cartoon/'))
            .toList();
        debugPrint(
  ...
  List<MatchingPair> _makePairsForCategory(String category) {
    ...
    if ((_isDifficultyVisible('easy')) &&
      ...
      if (assets != null && assets.isNotEmpty) {
        ...
        // If the user's selection resolves to Set 1 then return early so
      ...
      if ((category == 'Fruits' ||
        ...
        if (cartoons != null && cartoons.isNotEmpty) {
          ...
          if (difficulty == 'medium') {
            for (var i = 0; i < mediumIndices.length; i++) {
              items.add('$category-$difficulty-$i');
            }
          } else {
            for (var i = 0; i < hardIndices.length; i++) {
              items.add('$category-$difficulty-$i');
            }
          }
          continue;
    ...
    // If the current selection resolves to 'all', include stored custom-set
  ...
  Widget build(BuildContext context) {
      ...
      groups.forEach((difficulty, widgets) {
        ...
        if ((_selectedCategory == 'Fruits' ||
          ...
          if (cartoons != null && cartoons.isNotEmpty) {
            ...
            if (difficulty == 'medium') {
              for (var idx = 0; idx < mediumIndices.length; idx++) {
              ...
              for (var idx = 0; idx < hardIndices.length; idx++) {
    ...
    // If the user selected 'All' (or the selected difficulty maps to 'all'),
    // include visuals for stored custom sets belonging to this category so
    ...
    try {
      ...
      if (selected == 'all') {
        for (final key in _customSetImages.keys) {
          ...
          if (imgs == null || imgs.isEmpty) continue;
          for (var i = 0; i < imgs.length; i++) {
            final id = '$_selectedCategory-$key-$i';
