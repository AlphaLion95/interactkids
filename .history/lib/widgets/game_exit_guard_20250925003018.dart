...
class _GameExitGuardState extends State<GameExitGuard> {
  ...
  Widget build(BuildContext context) {
    // Use PopScope (supports Android predictive back). If PopScope isn't
    // available on an older SDK, this will still compile because PopScope is
    // part of the stable Flutter SDK in recent versions. If your SDK is
    // older, revert to WillPopScope manually.
    return PopScope(
      onWillPop: (details) async => await _onWillPop(),
      child: widget.child,
    );
  }
}
