...
class _MatchingPicturesScreenState extends State<MatchingPicturesScreen> {
  ...
  static final Map<String, List<Widget>> _categoryWidgets = {
    'Fruits': [
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.local_pizza, size: 100, color: Colors.red))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.brightness_1, size: 100, color: Colors.orange))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.circle, size: 100, color: Colors.purple))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.star, size: 100, color: Colors.yellow))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.favorite, size: 100, color: Colors.pink))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.local_pizza, size: 100, color: Colors.deepOrange))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.cake, size: 100, color: Colors.brown))),
      SizedBox(width: 120, height: 120, child: Center(child: Icon(Icons.icecream, size: 100, color: Colors.blueGrey))),
    ],
    'Vegetables': [
      Center(child: Icon(Icons.grass, size: 60, color: Colors.green)),
      Center(child: Icon(Icons.eco, size: 60, color: Colors.greenAccent)),
      Center(child: Icon(Icons.local_florist, size: 60, color: Colors.teal)),
      Center(child: Icon(Icons.spa, size: 60, color: Colors.lightGreen)),
    ],
    'Colors': [
          ...
          actions: [
            IconButton(
              ...
              onPressed: () async {
                ...
                if (!mounted) return;
                if (undone == true) {
                  messenger?.showSnackBar(const SnackBar(content: Text('Undid last match')));
                } else {
                  messenger?.showSnackBar(const SnackBar(content: Text('Nothing to undo')));
                }
              },
            ),
            IconButton(
              ...
              onPressed: () async {
                ...
                final confirmed = await showDialog<bool>(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    content: const Text('This will clear your progress for this game mode.'),
                    actions: [
                ...
                if (confirmed == true) {
                  ...
                  if (!mounted) return;
                  messenger?.showSnackBar(const SnackBar(content: Text('Progress reset')));
                }
              },
            ),
          ],
