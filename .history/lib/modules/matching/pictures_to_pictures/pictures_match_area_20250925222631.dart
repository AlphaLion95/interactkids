...
class DragMatchArea extends StatefulWidget {
  ...
  const DragMatchArea({required Key key, required this.leftItems, required this.rightItems, required this.buildLeft, required this.buildRight, required this.onProposeMatch, this.isDrawingNotifier}) : super(key: key);
...
class _DragMatchAreaState extends State<DragMatchArea> {
  ...
  void _handlePanStart(Offset globalPos) {
    ...
    final startedInMargin = local == null ? false : (local.dx <= marginPx || local.dx >= (width - marginPx));

    try {
      if (widget.isDrawingNotifier != null) widget.isDrawingNotifier!.value = !startedInMargin;
    } catch (_) {}
  ...
  void _handlePanEnd() async {
    ...
    try {
      if (widget.isDrawingNotifier != null) widget.isDrawingNotifier!.value = false;
    } catch (_) {}
  }
