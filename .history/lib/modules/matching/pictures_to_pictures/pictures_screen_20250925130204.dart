...
class _MatchingPicturesScreenState extends State<MatchingPicturesScreen> {
  ...
  List<MatchingPair> _makePairsForCategory(String category) {
    ...
    for (final entry in groups.entries) {
      ...
      if (_selectedDifficulty != null && _selectedDifficulty != 'all' && difficulty != _selectedDifficulty) continue;
  ...
  Widget build(BuildContext context) {
    ...
    final pairs = _selectedCategory == null ? <MatchingPair>[] : _makePairsForCategory(_selectedCategory!);
    final visuals = <String, Widget>{};
    final groups = _selectedCategory == null ? {} : _categoryWidgets[_selectedCategory!] ?? {};
    groups.forEach((difficulty, widgets) {
      if (_selectedDifficulty != null && _selectedDifficulty != 'all' && difficulty != _selectedDifficulty) return;
                      ...
                      child: AnimatedContainer(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(_displayLabel(cat), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          ],
                        ),
                      ),
                    );
                          ...
                          children: ['easy', 'medium', 'hard', 'all'].map((level) {
                            return GestureDetector(
                              ...
                              child: AnimatedContainer(
                                ...
                                height: 140,
                                decoration: BoxDecoration(
                                  color: _selectedDifficulty == level ? Colors.green.shade400 : Colors.white,
                                  borderRadius: BorderRadius.circular(16),
                                  boxShadow: const [BoxShadow(color: Colors.black12, blurRadius: 12)],
                                ),
                                child: Center(
                                  child: Text(level.toUpperCase(), style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _selectedDifficulty == level ? Colors.white : Colors.black)),
                                ),
                              ),
                            );
                          }).toList(),
