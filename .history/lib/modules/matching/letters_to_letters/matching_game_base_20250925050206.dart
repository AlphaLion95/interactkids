            ...
            onPointerDown: (ev) {
              ...
              if (areaState is _DragMatchAreaState) {
                try {
                  ...
                  if (local != null) {
                    final box = areaState.context.findRenderObject() as RenderBox?;
                    final w = box?.size.width ?? MediaQuery.of(areaState.context).size.width;
                    const margin = 120.0;
...
class _DragMatchArea extends StatefulWidget {
  ...
  const _DragMatchArea(
    ...
    this.isDrawingNotifier}) : super(key: key);
  @override
...
class _DragMatchAreaState extends State<_DragMatchArea>
  ...
  // Immediate drag implementation to forward updates from
  // ImmediateMultiDragGestureRecognizer into our pan handlers.
  class _ImmediateDrag implements Drag {
    final _DragMatchAreaState _state;
    _ImmediateDrag(this._state);
