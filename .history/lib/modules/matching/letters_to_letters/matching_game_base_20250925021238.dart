...
class _DragMatchAreaState extends State<_DragMatchArea> {
  ...
  void _startDrag(dynamic item, bool fromLeft, Offset globalPos) {
    setState(() {
      ...
      _startCenterLocal = _toLocal(_centerOfKey(fromLeft ? _leftKeys[item] : _rightKeys[item]));
      _points = [];
  ...
  void _updateDrag(Offset globalPos) {
    setState(() {
      ...
      // update hover target depending on drag direction (based on last point)
  ...
  void _endDrag() {
    if (_draggingItem == null || _points.isEmpty) {
      setState(() {
        _points = [];
        _startCenterLocal = null;
        _draggingItem = null;
        _hoverTarget = null;
      });
      return;
    ...
    // Check whether any point on the stroke comes close to a target on the opposite side
    dynamic hit;
    ...
    if (_draggingFromLeft) {
      for (final entry in _rightKeys.entries) {
        ...
        for (final pt in _points) {
          if ((centerLocal - pt).distance <= threshold) {
            hit = entry.key;
            break;
          }
        }
