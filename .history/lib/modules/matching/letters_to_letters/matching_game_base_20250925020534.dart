...
class _DragMatchAreaState extends State<_DragMatchArea> {
  ...
  /// Convert a global offset (window coordinates) into local coordinates
  /// relative to this widget's render box. Returns null if conversion fails.
  ...
  void _startDrag(dynamic item, bool fromLeft, Offset globalPos) {
    setState(() {
      ...
      // Convert the start and current points into local coordinates so
      // drawing with CustomPaint aligns with this widget's Stack.
      _start = _toLocal(_centerOfKey(fromLeft ? _leftKeys[item] : _rightKeys[item]));
      _current = _toLocal(globalPos);
      _hoverTarget = null;
    });
  }
    ...
    setState(() {
      ...
      if (_draggingFromLeft) {
        for (final entry in _rightKeys.entries) {
          ...
          if (centerLocal == null || _current == null) continue;
          if ((centerLocal - _current!).distance <= threshold) {
  ...
  void _endDrag() {
    ...
    if (_draggingFromLeft) {
      ...
      if (hit != null) {
        _attemptProposedMatch(_draggingItem, hit);
      }
    } else {
      for (final entry in _leftKeys.entries) {
        final centerLocal = _toLocal(_centerOfKey(entry.value));
      ...
      if (hit != null) {
        _attemptProposedMatch(hit, _draggingItem);
      }
    }
          ...
          if (_start != null && _current != null)
            CustomPaint(
              painter: _LinePainter(
                start: _start!,
                end: (_hoverTarget != null
                    ? (_toLocal(_centerOfKey(_draggingFromLeft ? _rightKeys[_hoverTarget] : _leftKeys[_hoverTarget])) ?? _current!)
                    : _current!),
              ),
            ),
