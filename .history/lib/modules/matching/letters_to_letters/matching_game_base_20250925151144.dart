...
class _DragMatchAreaState extends State<_DragMatchArea>
  ...
  final threshold = _hitThreshold;
    ...
    // Prefer segment-based checking for reliability. If there is only a
    // single recorded point (shouldn't happen due to earlier checks), fall
    // back to point-based checks.
    if (_points.length < 2) {
      ...
      for (var i = 0; i < _points.length - 1; i++) {
        ...
        // check lefts by distance to segment [a,b]
        for (final entry in _leftKeys.entries) {
          final c = _toLocal(_centerOfKey(entry.value));
          if (c == null) continue;
          ...
          if (dist <= threshold) {
            if (firstLeftIndex == null) {
              firstLeftIndex = i;
              firstLeftKey = entry.key;
              if (_showHitDebug) print('DragMatch: left hit ${entry.key} dist=${dist.toStringAsFixed(1)} segment=$i');
            }
          }
        }

        // check rights by distance to segment [a,b]
        for (final entry in _rightKeys.entries) {
          final c = _toLocal(_centerOfKey(entry.value));
          if (c == null) continue;
          final dist = _distancePointToSegment(c, a, b);
          if (dist <= threshold) {
            if (firstRightIndex == null) {
              firstRightIndex = i;
              firstRightKey = entry.key;
              if (_showHitDebug) print('DragMatch: right hit ${entry.key} dist=${dist.toStringAsFixed(1)} segment=$i');
            }
          }
        }
