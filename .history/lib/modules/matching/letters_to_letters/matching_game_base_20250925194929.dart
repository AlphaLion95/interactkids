...
class MatchingGameBaseState extends State<MatchingGameBase>
  ...
  // shared controller used to play a short 'pop' bounce for the tapped item.
  late final AnimationController _pulseController;
  late final Animation<double> _pulseAnim;
  late final Animation<double> _bounceAnim;
  // controller for continuous bounce when an item is selected
  ...
  // Notifier to indicate whether the user is currently drawing (prevents scroll)
  late final ValueNotifier<bool> _isDrawingNotifier;

  @override
  void initState() {
    super.initState();
    // prepare pulse controller: longer so we can run a ball-like bounce (multiple bounces)
    _pulseController = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 900));
    _pulseAnim =
        CurvedAnimation(parent: _pulseController, curve: Curves.easeOut);
    // Bounce animation uses a TweenSequence to produce a damped multi-bounce
    ...
    _pulseController.addStatusListener((s) {
      if (s == AnimationStatus.completed) {
        ...
        // clear the tracked pulse item so future taps can reuse
        setState(() {
          _pulseItem = null;
        });
      }
  ...
  // continuous looping bounce for selected items (used while selection remains)
  _selectedLoopController = AnimationController(
  ...
  _selectedLoopAnim = TweenSequence<double>([
    ...
    // drawing notifier used by the drag area
    _isDrawingNotifier = ValueNotifier<bool>(false);

    _loadProgress();
    // ensure selected loop does not run until an item is selected
    // (controller already created above)
  }
  ...
  void dispose() {
    ...
    try {
      _selectedLoopController.dispose();
    } catch (_) {}
    super.dispose();
  }
  ...
  void _onLeftTap(dynamic item) {
    // play pulse then select
    _triggerPulse(item);
    setState(() {
      // toggle selection: tapping the same item deselects
      if (selectedLeft == item) {
        selectedLeft = null;
        try {
          _selectedLoopController.stop();
        } catch (_) {}
      } else {
        selectedLeft = item;
        try {
          _selectedLoopController.repeat(reverse: true);
        } catch (_) {}
      }
    });
    if (selectedRight != null) {
      // wait for match removal until after pulse finishes
      _tryMatch();
    }
  }
  ...
  // Trigger a transient pulse for a tapped item: show glow and play the
  // bounce animation. The glow is driven by [_tapPulse] and cleared after
  // a short delay so AnimatedOpacity can transition smoothly.
  void _triggerPulse(dynamic item) {
    try {
      setState(() {
        _tapPulse[item] = true;
        _pulseItem = item;
      ...
      // Keep the glow visible a little longer than the controller so
      // the visual stays while the bounce plays.
      final keepMs = (_pulseController.duration?.inMilliseconds ?? 900) + 200;
      Future.delayed(Duration(milliseconds: keepMs), () {
        if (!mounted) return;
        setState(() {
          _tapPulse[item] = false;
        });
      });
    } catch (_) {}
  }
  ...
  Future<void> _tryMatch() async {
    ...
    if (pair.left != null) {
      // Wait for the pulse animation (forward + reverse) so the user sees the
      // bounce/glow before the matched tile is removed.
      final singleMs = _pulseController.duration?.inMilliseconds ?? 380;
      final waitMs = singleMs * 2;
    ...
    // stop continuous selected-loop when match attempt completes
    try {
      _selectedLoopController.stop();
    } catch (_) {}
    selectedLeft = null;
                        ...
                        child: LayoutBuilder(builder: (context, constraints) {
                          if (widget.mode.supportsDragMatch) {
                            return _DragMatchArea(
                              ...
                              tapPulse: _tapPulse,
                              onTapPulse: _triggerPulse,
                              isDrawingNotifier: _isDrawingNotifier,
                              preferredCellSize: 120.0,
                              bounceControllers: null,
                      ...
                      animation: Listenable.merge([
                        ...
                        _selectedLoopController]),
                                            builder: (context, child) {
                                              // base scale from selection
                                              final baseScale =
                                                  selectedLeft == item
                                                      ? 1.12
                                                      : 1.0;
                                              // pulse scale when this item was tapped
                                              final pulseScale = (_pulseItem ==
                                                      item)
                                                  ? (1.0 +
                                                      _pulseAnim.value * 0.32)
                                                  : 1.0;
                                              // vertical bounce: one-shot pulse or continuous loop when selected
                                              double translateY = 0.0;
                                              if (_pulseItem == item) {
                                                translateY = _bounceAnim.value;
                                              } else if (selectedLeft == item) {
                                                translateY = _selectedLoopAnim.value;
                                              }
...
class _DragMatchArea extends StatefulWidget {
  ...
  final Map<dynamic, bool>? tapPulse;
  final Map<dynamic, AnimationController>? bounceControllers;
  final void Function(dynamic)? onTapPulse;
...
class _DragMatchAreaState extends State<_DragMatchArea>
  ...
  // local continuous loop controller for selections inside the drag area
  late final AnimationController _dragSelectedLoopController;
  late final Animation<double> _dragSelectedLoopAnim;
  ...
  // local selected loop animation: gentle up/down while an item is selected
  _dragSelectedLoopController = AnimationController(
    vsync: this, duration: const Duration(milliseconds: 800));
  _dragSelectedLoopAnim = TweenSequence<double>([
    TweenSequenceItem(
    ...
    TweenSequenceItem(
      tween: Tween(begin: -10.0, end: 0.0)
        .chain(CurveTween(curve: Curves.easeInOut)),
      weight: 50),
  ]).animate(_dragSelectedLoopController);
  _dragSelectedLoopController.repeat(reverse: true);
  }
  ...
  _dragSelectedLoopController.dispose();
                                  ...
                                  Row(
                                    children: List.generate(perRow, (i) {
                                      ...
                                      return Padding(
                                        ...
                                        child: GestureDetector(
                                          onTap: () {
                                            // trigger pulse for tap inside drag-area
                                            try {
                                              widget.onTapPulse?.call(item);
                                            } catch (_) {}
                                            setState(() {
                                    ...
                                    children: List.generate(perRow, (i) {
                                      ...
                                      return Padding(
                                        ...
                                        child: GestureDetector(
                                          ...
                                          child: AnimatedBuilder(
                                            animation: _dragSelectedLoopController,
                                            builder: (context, child) {
                                              double translateY = 0.0;
                                              if (_selectedRight == item) {
                                                translateY = _dragSelectedLoopAnim.value;
                                              }
                                              return Transform.translate(
                                                offset: Offset(0, translateY),
                                                child: AnimatedScale(
