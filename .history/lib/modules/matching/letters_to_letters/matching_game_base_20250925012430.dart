...
class _DragMatchAreaState extends State<_DragMatchArea> {
  ...
  void _startDrag(dynamic leftItem, Offset globalPos) {
    setState(() {
      _draggingLeftItem = leftItem;
      _start = _centerOfKey(_leftKeys[leftItem]);
      _current = globalPos;
    });
  }
  ...
  void _endDrag() {
    if (_draggingLeftItem == null || _current == null) {
      setState(() { _start = null; _current = null; _draggingLeftItem = null; });
      return;
    }
    dynamic hitRight;
    for (final entry in _rightKeys.entries) {
      ...
      if ((center - _current!).distance <= 40) {
        hitRight = entry.key;
        break;
      }
    }
    if (hitRight != null) {
      widget.onMatched(_draggingLeftItem, hitRight);
    }
    setState(() { _start = null; _current = null; _draggingLeftItem = null; });
  }
      ...
      child: Stack(
        ...
        children: [
                ...
                child: Column(
                  ...
                  children: [
                    for (final item in widget.leftItems)
                      GestureDetector(
                        onPanStart: (d) => _startDrag(item, d.globalPosition),
                        child: Container(
                          key: _leftKeys[item],
          ...
          if (_start != null && _current != null)
            CustomPaint(
              painter: _LinePainter(start: _start!, end: _current!),
            ),
        ],
      ),
    );
