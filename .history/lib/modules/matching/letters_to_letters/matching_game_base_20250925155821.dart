...
class MatchingGameBaseState extends State<MatchingGameBase> with TickerProviderStateMixin {
  ...
  // animation controllers for tap bounce per item
  final Map<dynamic, AnimationController> _bounceControllers = {};
  ...
  Future<void> _loadProgress() async {
    ...
    // shuffle remaining items for a fresh layout independently so left
    // and right columns don't mirror each other.
  ...
  void _onLeftTap(dynamic item) {
    setState(() {
      // trigger a transient tap pulse visual centered on the item
      _triggerTapPulse(item);
      selectedLeft = item;
  ...
  // Trigger a transient pulse for a tapped item. The pulse state is
  // stored in [_tapPulse] and cleared after a short delay so Animated
  // widgets can play their transitions.
  void _triggerTapPulse(dynamic item) {
    try {
      // ensure we flip state so repeated taps re-trigger animations
      setState(() {
        _tapPulse[item] = true;
      });
      // create or reuse controller for bounce
      _bounceControllers.putIfAbsent(item, () => AnimationController(
        vsync: this,
        duration: const Duration(milliseconds: 400),
      )..addStatusListener((status) {
        if (status == AnimationStatus.completed) {
          _bounceControllers[item]?.reverse();
        } else if (status == AnimationStatus.dismissed) {
          // animation done, clean up if needed
        }
      }));
      _bounceControllers[item]?.forward(from: 0.0);
