...
class _DragMatchAreaState extends State<_DragMatchArea>
  ...
  void _handlePanEnd() {
    ...
    if (_points.length < 3) {
      ...
      return;
      double? firstLeftDist; 
      double? firstRightDist; 
      final List<Map<String, dynamic>> leftHits = [];
      final List<Map<String, dynamic>> rightHits = [];
  // left or right item center. Use distance from stroke segments to the
  // center point (more robust than sampling stroke points) so we don't
  // miss matches when the input samples skip over centers.
  final threshold = _hitThreshold;
    ...
    // We'll scan the whole stroke and pick, for each side, the earliest
    // segment where any target center is within threshold. If multiple
    // targets are found on the same segment, prefer the one with the
    // smaller distance (nearest). This avoids picking a later-appearing
    // target that happens to be checked first.
    double? firstLeftDist;
    double? firstRightDist;
    ...
    if (_points.length < 2) {
      for (var i = 0; i < _points.length; i++) {
        ...
        for (final entry in _leftKeys.entries) {
          ...
          if (c == null) continue;
          final dist = (c - pt).distance;
          if (dist <= threshold) {
            if (firstLeftIndex == null || i < firstLeftIndex ||
                (i == firstLeftIndex && (firstLeftDist == null || dist < firstLeftDist))) {
              firstLeftIndex = i;
              firstLeftKey = entry.key;
              firstLeftDist = dist;
              if (_showHitDebug) print('DragMatch: left candidate ${entry.key} dist=${dist.toStringAsFixed(1)} point=$i');
            }
          }
        }
        for (final entry in _rightKeys.entries) {
          ...
          final dist = (c - pt).distance;
          if (dist <= threshold) {
            if (firstRightIndex == null || i < firstRightIndex ||
                (i == firstRightIndex && (firstRightDist == null || dist < firstRightDist))) {
              firstRightIndex = i;
              firstRightKey = entry.key;
              firstRightDist = dist;
              if (_showHitDebug) print('DragMatch: right candidate ${entry.key} dist=${dist.toStringAsFixed(1)} point=$i');
            }
          }
        }
      }
        ...
        for (final entry in _leftKeys.entries) {
          ...
          if (dist <= threshold) {
            if (firstLeftIndex == null || i < firstLeftIndex ||
              ...
              firstLeftKey = entry.key;
              firstLeftDist = dist;
              if (_showHitDebug) print('DragMatch: left candidate ${entry.key} dist=${dist.toStringAsFixed(1)} segment=$i');
            }
          }
        }
        ...
        for (final entry in _rightKeys.entries) {
          ...
          if (dist <= threshold) {
            if (firstRightIndex == null || i < firstRightIndex ||
              ...
              firstRightKey = entry.key;
              firstRightDist = dist;
              if (_showHitDebug) print('DragMatch: right candidate ${entry.key} dist=${dist.toStringAsFixed(1)} segment=$i');
            }
          }
        }
      }
    ...
    if (_showHitDebug) {
      print('DragMatch: chosen left=$firstLeftKey idx=$firstLeftIndex dist=${firstLeftDist?.toStringAsFixed(1)} right=$firstRightKey idx=$firstRightIndex dist=${firstRightDist?.toStringAsFixed(1)}');
    }
