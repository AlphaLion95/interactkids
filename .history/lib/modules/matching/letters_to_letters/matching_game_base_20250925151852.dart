...
class _DragMatchAreaState extends State<_DragMatchArea>
  ...
  void _handlePanEnd() {
    ...
    for (final entry in _leftKeys.entries) {
      ...
      if (d <= threshold) leftHits.add({'idx': _points.length - 1, 'key': entry.key, 'dist': d});
    }
    for (final entry in _rightKeys.entries) {
      final c = _toLocal(_centerOfKey(entry.value));
      if (c == null) continue;
      final d = (c - lastPt).distance;
      if (d <= threshold) rightHits.add({'idx': _points.length - 1, 'key': entry.key, 'dist': d});
    }

    if (_showHitDebug) print('DragMatch: leftHits=${leftHits.length} rightHits=${rightHits.length}');
    ...
    for (final l in leftHits) {
      for (final r in rightHits) {
        if (l['key'] == r['key']) {
          ...
          if (chosenLeft == null || candidateScore < (currentScore ?? double.infinity)) {
            chosenLeft = l;
            chosenRight = r;
            ...
            final currentDist = (nonNullChosenLeft['dist'] as double) + (nonNullChosenRight['dist'] as double);
    ...
    if (_showHitDebug) print('DragMatch: chosenLeft=$chosenLeft chosenRight=$chosenRight');
    ...
    if (chosenLeft != null && chosenRight != null) {
      ...
      if (leftKey == rightKey) {
        ...
        if (_showHitDebug) print('DragMatch: ambiguous candidates left=$leftKey right=$rightKey; not proposing');
