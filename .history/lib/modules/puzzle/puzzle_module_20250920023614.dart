// lib/features/puzzle/puzzle_screen.dart
import 'dart:io';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          children: [
            // Top info: difficulty and reset button
            Padding(
                  ...
                  children: [
                    ...
                    Positioned.fill(
                      child: ClipRRect(
                        ...
                        child: ColorFiltered(
                          colorFilter: ColorFilter.mode(Colors.white.withOpacity(0.35), BlendMode.lighten),
                          child: Opacity(
                            opacity: 0.32,
                            child: Image(
                    ...
                    // puzzle board with tray overlayed
                    Center(
                      child: AspectRatio(
                        aspectRatio: 1,
                        child: ClipRRect(
            ...
            // Bottom tray: available pieces to drag into board
            SizedBox(
              height: 120,
              child: Container(
...
class _PuzzlePiece extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // We render the full image sized to (tileWidth * cols) x (tileHeight * rows),
    // then shift it with negative offsets so only the tile area shows.
      ...
      builder: (context, constraints) {
        ...
        return Stack(
          children: [
            ...
            // Clip to tile area (ensures only the tile shows)
            ClipRect(
...
class _Bubble {
  ...
  factory _Bubble.random() {
    ...
    return _Bubble(
      rand.nextDouble(),
      12 + rand.nextDouble() * 20,
      0.1 + rand.nextDouble() * 0.6,
      rand.nextDouble(),
      Colors.primaries[rand.nextInt(Colors.primaries.length)].withOpacity(0.12 + rand.nextDouble() * 0.18),
    );
  }
}
...
class _PuzzleLevelScreenState extends State<PuzzleLevelScreen> {
  ...
  Future<void> _addImage(String level) async {
    ...
    if (picked != null) {
      ...
      // Do NOT navigate to gameplay automatically
    }
  }
