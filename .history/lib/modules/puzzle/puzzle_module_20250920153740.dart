...
class _PuzzlePiece extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // For visual confirmation, use FlutterLogo if image fails
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return ClipRect(
          child: Image(
            key: ValueKey('img_${row}_${col}_${rows}_${cols}'),
            image: imageProvider,
            fit: BoxFit.cover, // scale image to cover the whole puzzle area
            width: pieceWidth * cols,
            height: pieceHeight * rows,
            alignment: Alignment(
              -1.0 + (2 * col + 1) / cols,
              -1.0 + (2 * row + 1) / rows,
            ), // focus on correct piece
