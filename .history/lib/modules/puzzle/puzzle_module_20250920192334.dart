        ...
        child: Listener(
          ...
          child: Stack(
            children: [
              ...
              // Main puzzle board and tray
              Center(
                child: AspectRatio(
                  aspectRatio: _imageAspectRatio ?? 1.5,
                  child: LayoutBuilder(
                    builder: (context, constraints) {
                      final boardWidth = constraints.maxWidth * 0.7;
                      final trayWidth = constraints.maxWidth * 0.25;
                      return Row(
                        children: [
                          // Puzzle box with border and background
                          Container(
                            width: boardWidth,
                            ...
                            child: Padding(
                              ...
                              child: _PuzzleBoardWithTray(
                                ...
                                onStartDraggingFromTray: (idx) {
                                  setState(() {
                                    _draggingPieceIdx = idx;
                                  });
                                },
                                onEndDragging: () {
                                  setState(() {
                                ...
                                for (final idx in pieceOrder)
                                  Padding(
                                    ...
                                    child: GestureDetector(
                                      onTap: () {
                                        // Optionally: implement tap-to-place
                                      },
                                      child: _trayPieceWidget(_imageProvider, idx),
                                    ),
                                  ),
                              ],
                            ),
              ...
              // Progress bar and reset button (top right)
              Positioned(
                top: 16,
                right: 24,
                child: Row(
                  children: [
                    SizedBox(
                      width: 180,
                      child: LinearProgressIndicator(
                        value: boardState.where((e) => e != null).length / (rows * cols),
              ...
              // Floating Back Button (top left, pillow style)
              Positioned(
                top: 16,
                left: 16,
                child: Material(
                  ...
                  child: InkWell(
                    ...
                    child: const Padding(
                      ...
                      child: Icon(Icons.arrow_back, size: 28, color: Colors.deepOrange),
                    ),
                  ),
                ),
              ),
...
class ConfettiAndBalloonsOverlay extends StatefulWidget {
  ...
  @override
  State<ConfettiAndBalloonsOverlay> createState() =>
      _ConfettiAndBalloonsOverlayState();
}
...
class _Balloon {
  ...
  _Balloon copyWith({bool? popped}) =>
      _Balloon(x, speed, initY, color, popped: popped ?? this.popped);
}
...
class _ConfettiParticle {
  ...
  final Color color;
  _ConfettiParticle(
      this.x, this.y, this.speed, this.angle, this.size, this.color);
...
class _ConfettiPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final c in confetti) {
      ...
      canvas.drawRect(
          Rect.fromCenter(
              center: Offset.zero, width: c.size, height: c.size * 0.4),
          paint);
      canvas.restore();
    }
  }
