...
class _PuzzleScreenState extends State<PuzzleScreen> {
  ...
  Widget _trayPieceWidget(ImageProvider provider, int pieceIdx) {
    // The aspect ratio of a single tile: (image width / cols) / (image height / rows)
    // = (image width / image height) * (rows / cols)
    final double tileAspect = (cols > 0 && rows > 0) ? (_imageAspectRatio ?? 1.0) * rows / cols : 1.0;
    // Use a fixed height for tray pieces, width based on tile aspect
    const double trayPieceHeight = 56; // consistent small size
