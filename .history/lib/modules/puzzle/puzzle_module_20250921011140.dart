...
class _PuzzleTheme {
  ...
  // Stub for tray piece widget
  Widget _trayPieceWidget(ImageProvider provider, int pieceIdx) {
    return Container(
      width: 48,
      height: 48,
      color: Colors.grey.shade200,
      child: Center(child: Text('Piece $pieceIdx')),
    );
  }
  // Tray drag callbacks
  void Function(int)? onStartDraggingFromTray;
  void Function()? onEndDragging;
// ...existing code...
  // --- STUBS FOR ALL REQUIRED FIELDS ---
  int rows = 3;
  int cols = 3;
  List<int?> boardState = List<int?>.filled(9, null);
  List<int> pieceOrder = List<int>.generate(9, (i) => i);
  int? draggingIndex;
  double? _imageAspectRatio = 1.0;
  ImageProvider _imageProvider = const AssetImage('assets/puzzle/easy1.png');
  int? _highlightedSlotIdx;
  bool hasWon = false;
  Map<int, GlobalKey> _slotKeys = {};
  Offset? _dragGlobalPosition;
  int? _draggingPieceIdx;
  void _onPieceDroppedToBoard(int slot, int piece) {}
  void _onPieceRemovedFromBoard(int slot) {}
  void _resetGame() {}
...
// --- STUB FOR ConfettiAndBalloonsOverlay IF MISSING ---

// --- STUB FOR ConfettiAndBalloonsOverlay ---
class ConfettiAndBalloonsOverlay extends StatelessWidget {
  final bool show;
  const ConfettiAndBalloonsOverlay({super.key, required this.show});
  @override
  Widget build(BuildContext context) {
    return show
        ? IgnorePointer(
