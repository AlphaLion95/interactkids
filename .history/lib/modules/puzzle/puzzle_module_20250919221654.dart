      ...
      DeviceOrientation.landscapeRight,
                    builder: (context, constraints) {
                      return DragTarget<int>(
                        ...
                        onAccept: (data) {
                          setState(() {
                            int? boardIdx = boardState.indexOf(data);
                            if (boardIdx != -1) {
                          ...
                          return GestureDetector(
                            ...
                            onVerticalDragUpdate: (details) {
                              if (trayScrollController.hasClients) {
                                final newOffset = trayScrollController.offset - details.delta.dy;
                                trayScrollController.jumpTo(
                                  newOffset.clamp(
                                    0.0,
    ...
    super.dispose();
                ...
                Expanded(
                  child: Center(
                    child: LayoutBuilder(
                      builder: (context, constraints) {
                        // Make the board larger and centered
                        final double boardHeight = constraints.maxHeight * 0.98;
                        ...
                        return AspectRatio(
                          ...
                          child: SizedBox(
                            ...
                            child: Stack(
                              children: [
                                // Guide image with higher brightness and no bubbles in front
                                ColorFiltered(
                                  colorFilter: ColorFilter.mode(
                                    Colors.white.withOpacity(0.35), // brighten the guide image
                                    BlendMode.lighten,
                                  ),
                                  child: Opacity(
                                ...
                                // Puzzle grid (no bubbles in front)
                                _PuzzleBoardWithTray(
                                  ...
                                  onPieceDropped: (boardIdx, pieceIdx) {
                                    setState(() {
                                      // Remove the piece from its previous location (board or tray)
                                      // Remove from previous board position if present
                                      int? prevBoardIdx = boardState.indexOf(pieceIdx);
                                      if (prevBoardIdx != -1) {
                                        boardState[prevBoardIdx] = null;
                                      }
                                      // Remove from tray if present
                                      pieceOrder.remove(pieceIdx);
                                      // Place on new board position
                                  ...
                                  onPieceRemoved: (boardIdx) {
                                    setState(() {
                                      int? pieceIdx = boardState[boardIdx];
                                      if (pieceIdx != null) {
                                        boardState[boardIdx] = null;
                ...
                Container(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Reset', style: TextStyle(fontFamily: 'Nunito', color: Colors.white)),
