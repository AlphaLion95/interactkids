...
class PuzzleTypeScreen extends StatelessWidget {
  final List<PuzzleTheme> types = const [
    PuzzleTheme('Sea', Icons.waves, Color(0xFF40c4ff)),
    PuzzleTheme('Jungle', Icons.park, Color(0xFF66bb6a)),
    PuzzleTheme('Flying', Icons.flutter_dash, Color(0xFFb39ddb)),
  ];
          ...
          children: [
            ...
            Material(
              ...
              child: InkWell(
                ...
                splashColor: Colors.orange.withAlpha((0.18 * 255).round()),
                highlightColor: Colors.orange.withAlpha((0.10 * 255).round()),
                child: Container(
                  width: 96,
                  height: 96,
...
class _PuzzleScreenState extends State<PuzzleScreen> {
  ...
  void initState() {
    ...
    cols = widget.cols;
    boardState =
        widget.initialBoardState ?? List<int?>.filled(rows * cols, null);
    // If restoring a saved board state, tray should contain all pieces not on the board
    if (widget.initialPieceOrder != null) {
      pieceOrder = List<int>.from(widget.initialPieceOrder!);
    } else {
      // If boardState is not empty, tray = all pieces not on board
      final allPieces = List<int>.generate(rows * cols, (i) => i);
      final onBoard = boardState.whereType<int>().toSet();
      pieceOrder = allPieces.where((i) => !onBoard.contains(i)).toList();
    }
    _slotKeys = List.generate(rows * cols, (index) => GlobalKey());
    if (widget.imagePath != null) {
      _imageProvider = (widget.imagePath!.startsWith('assets/')
  ...
  void _resetGame() {
    ...
    widget.onProgress
        ?.call(0.0, boardState: boardState, pieceOrder: pieceOrder);
  }

  void _onPieceDroppedToBoard(int slotIdx, int pieceIdx) {
    setState(() {
      if (boardState[slotIdx] == null) {
        if (pieceOrder.contains(pieceIdx)) {
          // Piece is from tray
          boardState[slotIdx] = pieceIdx;
          pieceOrder.remove(pieceIdx);
        } else {
          // Piece is from another board slot
  ...
  void _onPieceRemovedFromBoard(int slotIdx, int pieceIdx) {
    setState(() {
      if (boardState[slotIdx] == pieceIdx) {
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      widget.type.color.withAlpha((0.95 * 255).round()),
                      Colors.white.withAlpha((0.85 * 255).round()),
                      widget.type.color.withAlpha((0.85 * 255).round()),
                    ],
                    begin: Alignment.topLeft,
                    end: Alignment.bottomRight,
                  ),
                  ...
                  boxShadow: [
                    BoxShadow(
                      color: widget.type.color.withAlpha((0.25 * 255).round()),
                      blurRadius: 32,
                      spreadRadius: 4,
                  ...
                  border: Border.all(
                    color: widget.type.color.withAlpha((0.7 * 255).round()),
                    width: 4,
                  ),
                ),
                child: Row(
                      ...
                      builder: (context, child) {
                        return Transform.scale(
                          ...
                          child: Icon(widget.type.icon,
                              size: 54, color: Colors.white.withAlpha((0.95 * 255).round())),
                        );
                      },
                    ),
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            shadows: [
                              Shadow(
                                color: widget.type.color.withAlpha((0.4 * 255).round()),
                                blurRadius: 8,
...
class PuzzleTheme {
  ...
  final IconData icon;
  final Color color;
  const PuzzleTheme(this.name, this.icon, this.color);
}
