...
class _PuzzleLevelScreenState extends State<PuzzleLevelScreen> {
  ...
  Future<void> _loadProgress() async {
    ...
    final userImagesStr = prefs.getString('puzzle_userImages_${widget.type.name}');
    final boardStatesKey = 'puzzle_boardStates_${widget.type.name}';
    ...
    if (userImagesStr != null) {
      ...
      userImages = decoded.map((level, list) => MapEntry(level, List<String>.from(list)));
    }
    if (boardStatesStr != null) {
      ...
      _boardStates = decoded.map((level, imgMap) => MapEntry(
        level,
        (imgMap as Map<String, dynamic>).map((img, state) => MapEntry(img, state as Map<String, dynamic>)),
      ));
    } else {
      _boardStates = {};
    }
  ...
  void _onImageTap(String level, String imagePath) {
    ...
    List<int?>? boardState;
    List<int>? pieceOrder;
    if (_boardStates[level] != null && _boardStates[level]![imagePath] != null) {
      final state = _boardStates[level]![imagePath]!;
      boardState = (state['boardState'] as List).map((e) => e == null ? null : e as int).toList();
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) => PuzzleScreen(
          ...
          rows: gridSize,
          cols: gridSize,
          onProgress: (percent, {List<int?>? boardState, List<int>? pieceOrder}) async {
            setState(() {
...
class _PuzzleScreenState extends State<PuzzleScreen> {
  ...
  void _updateProgress() {
    ...
    if (widget.onProgress != null) {
      widget.onProgress!(percent, boardState: List<int?>.from(boardState), pieceOrder: List<int>.from(pieceOrder));
    }
  }
