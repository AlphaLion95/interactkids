...
class _PuzzleBoardWithTrayState extends State<PuzzleBoardWithTray> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      // Remove highlighted index update from build
              ...
              if (pieceIdx != null) {
                return Draggable<int>(
                  ...
                  onDragUpdate: (details) {
                    ...
                    // Update highlight index here
                    final boardBox = context.findRenderObject() as RenderBox?;
                    Offset? boardLocal;
                    if (boardBox != null) {
                      boardLocal =
                          boardBox.globalToLocal(details.globalPosition);
                      final double tileWidth =
                          boardBox.size.width / widget.cols;
                      final double tileHeight =
                          boardBox.size.height / widget.rows;
                      int? newHighlightedIndex;
                      for (int index = 0;
                          index < widget.rows * widget.cols;
                          index++) {
                        if (widget.boardState[index] != null) continue;
                        final row = index ~/ widget.cols;
                        final col = index % widget.cols;
                        final rect = Rect.fromLTWH(col * tileWidth,
      ...
      return Listener(
        onPointerMove: (event) {
          ...
          Offset? boardLocal;
          if (boardBox != null) {
            boardLocal = event.localPosition;
