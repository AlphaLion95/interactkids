...
class _PuzzleBoardWithTrayState extends State<PuzzleBoardWithTray> {
  ...
  void _updateHighlight(Offset globalPosition) {
    ...
    final double tileWidth = boardBox.size.width / widget.cols;
    final double tileHeight = boardBox.size.height / widget.rows;
    ...
    for (int index = 0; index < widget.rows * widget.cols; index++) {
      ...
      // treat dragged piece roughly same size as a tile
      final pieceRect = Rect.fromCenter(
          center: boardLocal, width: tileWidth, height: tileHeight);

      if (pieceRect.overlaps(rect)) {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      // Notify parent which slot is highlighted
      ...
      for (int index = 0; index < widget.rows * widget.cols; index++) {
        ...
        children.add(Positioned(
          ...
          child: DragTarget<int>(
            // we don’t use onAccept anymore, drop happens in onDragEnd
                ...
                return Draggable<int>(
                  ...
                  onDragEnd: (details) {
                    if (_highlightedIndex != null &&
                        widget.onPieceDropped != null) {
                      widget.onPieceDropped!(_highlightedIndex!, pieceIdx);
                    }
      ...
      return Listener(
        onPointerMove: (event) {
          _updateHighlight(event.position); // global position
        },
        onPointerUp: (_) {},
