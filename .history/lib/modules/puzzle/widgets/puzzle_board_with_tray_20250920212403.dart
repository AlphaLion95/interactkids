      ...
      for (int index = 0; index < widget.rows * widget.cols; index++) {
        ...
        children.add(Positioned(
          ...
          child: DragTarget<int>(
            ...
            onAccept: (data) {
              if (widget.onPieceDropped != null) widget.onPieceDropped!(index, data);
            },
            builder: (context, candidateData, rejectedData) {
              if (pieceIdx != null) {
                return Draggable<int>(
                  ...
                  onDragStarted: () {
                    // Set initial drag position to the center of the tray piece
                    final renderBox = context.findRenderObject() as RenderBox?;
                    Offset? localPos;
                    if (renderBox != null) {
                      localPos = renderBox.size.center(Offset.zero);
                    }
                    setState(() {
                  ...
                  onDragUpdate: (details) {
                    // Update drag position as the piece is dragged
                    setState(() {
                  ...
                  child: GestureDetector(
                    onDoubleTap: () {
                      if (widget.onPieceRemoved != null) widget.onPieceRemoved!(index);
                    },
                    child: SizedBox(
                      width: tileWidth,
                      height: tileHeight,
                      child: RepaintBoundary(
                        key: ValueKey('board_rb_${index}_${widget.rows}_${widget.cols}'),
                        child: PuzzlePiece(
                          key: ValueKey('board_${index}_${widget.rows}_${widget.cols}'),
                          imageProvider: widget.imageProvider,
                          rows: widget.rows,
                          cols: widget.cols,
                          row: pieceIdx ~/ widget.cols,
