...
class _PuzzleLevelScreenState extends State<PuzzleLevelScreen> {
  // Example: 3 levels, each with 5 images
  ...
  void initState() {
    ...
    imagesByLevel = {
      for (final level in levels)
        level: List.generate(5, (i) => 'assets/puzzle/
           {widget.type.name.toLowerCase()}_${level.toLowerCase()}_$i.png'),
    };
  }
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, imgIdx) {
                              ...
                              return _PuzzleImageTile(
                                imagePath: imgPath,
                                progress: 0.0, // TODO: wire up progress
                                onTap: () {
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
...
// --- PUZZLE GAME SCREEN STUB ---
class PuzzleScreen extends StatelessWidget {
  final String imagePath;
  final int rows;
  final int cols;
  final bool solved;
  const PuzzleScreen({Key? key, required this.imagePath, required this.rows, required this.cols, this.solved = false}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // TODO: Implement puzzle gameplay UI
    return Scaffold(
      appBar: AppBar(title: const Text('Puzzle')),
      body: Center(child: Text('Puzzle for $imagePath ($rows x $cols)')),
    );
  }
}
