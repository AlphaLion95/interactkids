...
// Global RouteObserver for the app
final RouteObserver<PageRoute> routeObserver = RouteObserver<PageRoute>();
  ...
  void _onImageTap(String level, String imagePath) async {
    ...
    final bool isDone = (progress[level]?[imagePath] ?? 0.0) >= 0.999;
...
class _PuzzleScreenState extends State<PuzzleScreen> {
  ...
  void _resetGame() {
    if (widget.solved) {
      // All pieces placed in correct order
      pieceOrder = [];
      boardState = List.generate(rows * cols, (i) => i);
      hasWon = true;
    } else {
